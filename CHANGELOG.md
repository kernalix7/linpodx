...
**Web UI reaches feature parity with the CLI.** A follow-up wave (previously tracked as Phases 26–27) rounds the web UI out to Docker-Desktop-class coverage: a container create wizard, a real-time event feed, an image-pull modal with a healthcheck inspector, Stacks and Pods tabs (plus a new `linpodx pod` CLI surface), a resource-limit editor, network IPAM detail with attach/disconnect, volume detail, and a Secrets tab. The web UI's information architecture was also reworked twice over (UI v6 grouped sidebar + dashboard hero, then v7's screenshot-verified dashboard/topbar pass) alongside four correctness fixes surfaced by end-to-end testing.
...
- **Container create wizard**: `POST /api/v1/containers/create` (bearer-gated, backed by the existing `ContainerCreate` IPC arm) plus a multi-section modal — image, name, repeatable port/env/volume rows, network select, labels, restart policy, inline validation, in-modal error state.
- **Real-time daemon event feed** on the dashboard: a new `LiveEvents` component subscribes over one `/ipc` socket to all eight container-lifecycle topics (new `ws::subscribe_multi`), keeps a 200-entry newest-first ring with color-coded kind badges, pause-on-hover with side-buffer flush, an auto-scroll toggle, and an accent highlight when the event's container is open in the detail drawer.
- **Image pull modal + healthcheck inspector**: a "Pull image" modal drives the existing `ImagePullJob` async surface over `/ipc` (live progress lines, terminal success/failure) with a reclaimable-space badge next to prune; container detail gains a healthcheck section on Overview (command, interval/timeout/retries/start-period, current health chip, empty state).
- **Pod support end to end**: `Method::{PodList,PodCreate,PodStart,PodStop,PodRemove}` IPC arms, a `pod.rs` podman pod wrapper (`ps`/`create`/`start`/`stop`/`rm`), bearer-gated REST routes (`GET /pods`, `POST /pods/create`, `POST /pods/:id/{start,stop,remove}`), a Pods tab (status chip, container count, infra id, stack badge, create-pod modal), and `linpodx pod {ls,create,start,stop,rm}` on the CLI. `ContainerSummary` gains an additive `labels` field, parsed from `podman ps`'s `Labels` object.
- **Stacks tab**: containers grouped by compose project label (`com.docker.compose.project` → `io.podman.compose.project` → standalone), with a per-stack running/total chip, member table, bulk start/stop/restart with progress toasts, and a stack badge + filter on the containers table.
- **Resource-limit editor, network IPAM, volume detail, Secrets tab**: container detail gains an Edit-limits section (memory/cpus/pids/restart policy → `POST /containers/:id/update`); networks gain an expanding detail view (subnets/gateway/dns, member table with per-member Disconnect, attach-container select); volumes gain an accordion detail (mountpoint copy, driver, created, human size, in-use chips); a new Secrets tab supports list/create/remove (the value is cleared from the form on close).
- New IPC arms backing the above: `ContainerUpdate`, `NetworkInspectDetail`/`NetworkConnect`/`NetworkDisconnect`, `VolumeInspectDetail`, `SecretList`/`SecretCreate`/`SecretRemove` (the richer inspects are suffixed `*Detail`; the existing lean `NetworkInspect`/`VolumeInspect` variants are frozen for wire compat), backed by a podman `update` wrapper (only `Some` fields become flags), network inspect/connect/disconnect, volume `inspect_detail` (size via `system df -v`, in-use via `ps --filter volume=`), and secret `ls`/`create`/`rm` — the secret value travels to podman over stdin, never argv, and secrets audit records the name only.
- UI v6: sidebar regrouped into 5 labeled collapsible sections (Home / Workloads / Resources / AI Sandbox / System) with per-section accents and persisted collapse; topbar gains a section›page breadcrumb, a Cmd-K hint chip, and a global "+ Create" split-button; dashboard hero v2 adds a disk-capacity donut, CPU/Mem area charts, and a live-events + quick-actions row; a new Disk management center (per-category usage bars, reclaimable callouts, one-click prune); log search with highlight/match-nav and download-as-file; 6 self-authored empty-state illustrations.
- UI v7: dashboard charts and the live-events feed backfill on mount instead of opening empty; the counts row becomes clickable mini-cards plus a Top-consumers table (per-row sparklines, opens the detail drawer); `?tab=<slug>` URL routing alongside the existing `#container/<id>` deep link; right-click context menus on all seven entity tables with `j`/`k`/`Enter` keyboard navigation.
...
- Web UI design-review pass (v6.1): the sidebar footer no longer claims "read-only — use CLI to mutate" (stale v1-era copy still present after the UI gained mutating actions); it now shows the live daemon version. Page heads open with a section-tinted gradient wash, the collapsed 60px rail keeps section-qualified tooltips, the topbar gains a daemon health pill (dot + running/total), and cards/tiles get a 1px hover lift (suppressed under `prefers-reduced-motion`).
- Web UI density pass (v6.2): Docker-Desktop-style two-line primary cells (name + muted image/id sub-line) across the five entity tables, with a persisted comfortable/compact density toggle in the topbar.
- Web UI interaction pass (v7): the topbar's daemon status and token-set indicators are merged into fewer elements with an icon-only density cycle below 1100px; button SVGs are capped at 16px; long image references truncate with a tooltip instead of overlapping the status column.
- `linpodx-daemon` caches the `K8sAdapter` across requests instead of re-parsing the kubeconfig and re-negotiating TLS on every one of the six k8s dispatch arms; a broken kubeconfig no longer needs to fail every call before a fix takes effect.
...
- **WS clients that pre-authenticate (mTLS, header, subprotocol, or query token) no longer get a spurious `-32700`**: the CLI always sends a legacy `{"auth":<token>}` first frame for old-daemon compatibility, but the daemon skipped draining it once the socket was already authenticated by another mechanism, so the stale frame reached the RPC loop and its error response landed where the first real reply belonged (breaking mTLS/remote version round-trips). Pre-authenticated connections now drain at most one frame matching a conservative legacy-auth classifier (a real RPC call that happens to carry an `auth` argument is never swallowed).
- **GPU passthrough**: `--group-add render/video` by group *name* failed deterministically on images whose `/etc/group` lacks that entry (e.g. Alpine). Host group names are now resolved to numeric GIDs via `/etc/group`; a missing host group skips the flag with a warning instead of failing container creation.
- **`linpodx network egress set --profiles-dir`** now actually applies: the handler read `LINPODX_SANDBOX_PROFILES_DIR` directly, but `clap` only falls back to the env var when no flag is given, so a flag-only invocation silently wrote to the wrong directory while `status` read the real one (permanent `kind=none` mismatch). The resolved `cli.profiles_dir` is now threaded through like the other profile-aware handlers, with the precedence rule extracted into a tested `resolve_profiles_dir()` helper.
- Web UI: 20 undefined CSS classes — the entire container-detail-drawer chrome, chart hover layers, three tab wrappers, and two checkbox rows rendered as raw unstyled HTML — are now defined or renamed onto real classes, closing out the last visibly-broken regions from the v6 redesign; a compound-aware lint now gates the crate at zero undefined classes. Chart tooltips are `pointer-events: none`, fixing a hover flicker. Responsive rules now cover every v6 class at the 1100/900/720px breakpoints (hero stacks, management grid collapses to one column, drawer goes full-width, topbar chips shed their text) and the light-theme color wash was recalibrated.
- Cluster: the k8s-scale e2e test aborted the whole `--ignored` sweep with `NotFound` on any host with a live cluster but no pre-provisioned `linpodx-scale-fixture` deployment; it now soft-skips on `NotFound` with a note and still fails on any other error.
